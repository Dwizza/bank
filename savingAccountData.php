...
<div class="container mx-auto p-6">
        <h2 class="text-center text-2xl font-bold mb-6">Table des Comptes Bancaires(Saving Account)</h2>
        ...
        <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
            <thead>
                <tr class="bg-gray-200">
                    ...
                    <th class="py-3 px-6 text-left text-sm font-semibold text-gray-700">Taux d'intérêt (%)</th>
            ...
            <tbody>
                <?php 
                    ...
                    $affich = new Account("savingsaccount");
                    ...
                    foreach($result as $row){
                        ...
                        echo "<td class='py-3 px-6 text-sm text-gray-700'>".$row['Taux_Interet']."</td>";
                        echo "<td class='py-3 px-6 text-sm text-gray-700'>
                                <button onclick='magic(event)' class='text-blue-500 hover:text-blue-700'>Editer</button>
                            </td>";
                        echo "<td class='py-3 px-6 text-sm text-gray-700'>
    ...
    <script>
        function magic(event){
            ...
            inputTX.setAttribute("name","taux");
    ...
    <?php 
    include_once "./classes/SavingsAccount.php";
    if(isset($_POST['submit'])){
        $id = $_POST['ID'];
        $N_C = $_POST['N_C'];
        $balance = $_POST['balance'];
        $taux = $_POST['taux'];
        $update = new SavingsAccount($N_C,$balance,$taux);
        $update->Update($id);
